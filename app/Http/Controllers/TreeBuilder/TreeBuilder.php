...
class TreeBuilder implements TreeBuilderInterface
{
    ...
    {
        $linkHref = '/tree/node/'.$item['id'];
        $title = $item[$aliases['title']];
		
		/* if (empty($item['avatar'])){
					 $item['avatar'] = '/upload/tree_avatar.svg';
				 }else{
					 $avatar = cmsModel::yamlToArray($item['avatar']);
					 $item['avatar'] = '/upload/'.$avatar['normal'];
				 }*/

        // html
        $html = '';
        $html .= '<li class="tree-item li-tree-'.$item['id'].'">';
        $html .= ('<a data-teip="'.@$item['teip'].'" data-spouce="'.@$item['spouce_id'].'" class="node spouce'.@$item['spouce_id'].' teip'.$item['teip'].' level'.$item['level'].' id'.$item['id'].' parent'.$item['parent'].'" href="' . $linkHref . '" data-id="'.$item['id'].'">
						<span class="add-parent" title="Добавить отца">+</span>
                        <span class="add-child" title="Добавить ребёнка">+</span>
						<span class="tree_avatar">'.@$item['avatar'].'
						</span> 
	...
	public function childTNode($item, $childNodes, $aliases, $nestingLevel, $userParams)
    {
        $title = $item[$aliases['title']];
	...
	private function makeRecursiveVisualTeipsTree($arr, $firstStart = null, $nestingLevel = 0)
    {
        $nodes = '';
        $ln = count($arr);
        ...
        while ($ln --) {
            ...
            if (isset($arr[$ln][$this->aliases['child']])) {
                $this->nestingLevel ++;
                $childNodes = $this->makeRecursiveVisualTeipsTree($arr[$ln][$this->aliases['child']], false, $this->nestingLevel);
            }

            $nodes .= $this->childTNode($arr[$ln], $childNodes, $this->aliases, $nestingLevel, $this->userParams);
        }
	...
	public function showTeipsTree()
    {
        if ($this->html) {
            return $this->html;
        } else {
            return $this->makeRecursiveVisualTeipsTree($this->getTree(), true);
        }
    }
